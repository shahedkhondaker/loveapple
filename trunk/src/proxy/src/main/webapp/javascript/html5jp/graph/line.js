...
html5jp.graph.line.prototype.draw = function(items, inparams) {
	...
	if( typeof(params.hLineWidth) == "number" && params.hLineWidth > 0 ) {
		...
		for(var i=1; i<params.y.length; i++) {
			...
			var aline_y0 = Math.round( cpos.y0 - cpos.h * ( params.y[i] - params.yMin ) / v_range );
			var aline_x1 = cpos.x1;
			this._draw_w_aline(aline_x0, aline_y0, aline_x1, params.hLineWidth, h_line_type, params.hLineColor);
		}
	}
	/* 各項目のデフォルト色を定義 */
...
垂直補助線を描画 TODO
* ---------------------------------------------------------------- */
html5jp.graph.line.prototype._draw_w_aline = function(x0, y0, y1, width, type, color) {
	color = this._csscolor2rgb(color);
	this.ctx.beginPath();
	color = "rgb(" + color.r + "," + color.g + "," + color.b + ")"
	this.ctx.strokeStyle = color;
	...
	if(type == "solid") {
		...
		while(1) {
			if(y + width*2 > y1) { break; }
			this.ctx.arc(y0, y, width/2, 0, Math.PI*2, false);
			this.ctx.fill();
			if(y + width*2 > y1) { break; }
			y = y + width*2;
		}
